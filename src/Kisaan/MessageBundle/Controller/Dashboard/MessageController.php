...
namespace Kisaan\MessageBundle\Controller\Dashboard;
...
use Kisaan\MessageBundle\Repository\MessageRepository;
...
class MessageController extends Controller
{
     ...
     *
     * @Method("GET")
     * @Route("/{page}", name="kisaan_dashboard_message", requirements={"page" = "\d+"}, defaults={"page" = 1})
     *
    ...
    public function indexAction(Request $request, $page)
    {
        ...
        $threadManager = $this->get('kisaan_message.thread_manager');
        $threads = $threadManager->getListingInboxThreads($user, $userType, $page);

        return $this->render(
            'KisaanMessageBundle:Dashboard/Message:inbox.html.twig',
            array(
                'threads' => $threads,
                'pagination' => array(
                    'page' => $page,
     ...
     *
     * @Route("/{slug}/new", name="kisaan_dashboard_message_new", requirements={
     *      "slug" = "[a-z0-9-]+$"
     * })
     *
     * @Method({"GET", "POST"})
     ...
     * @ParamConverter("listing", class="Kisaan\CoreBundle\Entity\Listing", options={"repository_method" = "findOneBySlug"})
     *
     * @param Request $request
    ...
    public function newThreadAction(Request $request, Listing $listing = null)
    {
        ...
        if ($message) {
            ...
            $session->getFlashBag()->add(
                'success',
                $translator->trans('message.new.success', array(), 'kisaan_message')
            );

            $url = $this->generateUrl('kisaan_dashboard_message_new', array('slug' => $listing->getSlug()));

            return $this->redirect($url);
        } elseif ($form->isSubmitted() && !$form->isValid()) {
            $session->getFlashBag()->add(
                'error',
                $translator->trans('message.new.error', array(), 'kisaan_message')
            );
        }

        return $this->render(
            'KisaanMessageBundle:Dashboard/Message:new_thread.html.twig',
            array(
     ...
     *
     * @Route("/conversation/{threadId}", name="kisaan_dashboard_message_thread_view", requirements={"threadId" = "\d+"})
     *
     * @param Request $request
     * @param int $threadId
     * @return RedirectResponse|Response
    ...
    public function threadAction(Request $request, $threadId)
    {
        ...
        $selfUrl = $this->generateUrl(
            'kisaan_dashboard_message_thread_view',
        ...
        return $this->render(
            'KisaanMessageBundle:Dashboard/Message:thread.html.twig',
            array(
                'form' => $form->createView(),
                'thread' => $thread,
            )
     ...
     *
     * @Route("/delete/{threadId}", name="kisaan_dashboard_message_thread_delete", requirements={"threadId" = "\d+"})
     *
    ...
    public function deleteAction($threadId)
    {
        ...
        return new RedirectResponse($this->generateUrl('kisaan_dashboard_message'));
    }

    /**
     * Get number of unread messages for user
     *
     * @Route("/get-nb-unread-messages", name="kisaan_dashboard_message_nb_unread")
     * @Method("GET")
     *
     * @param Request $request
     * @return JsonResponse
    ...
    {
        return $this->get('doctrine')->getManager()->getRepository('KisaanMessageBundle:Message');
    }
}
