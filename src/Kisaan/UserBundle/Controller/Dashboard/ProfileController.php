...
use Kisaan\UserBundle\Form\Type\ProfileSwitchFormType;
...
class ProfileController extends Controller
{
     ...
     * Switch profile
     *
     * @Route("/profile-switch", name="kisaan_user_dashboard_profile_switch")
     * @Method({"GET", "POST"})
     *
    ...
    public function profileSwitchAction(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $response = $this->redirect($this->generateUrl('kisaan_dashboard_message'));
        ...
        $nbMessages = $this->get('doctrine')->getManager()->getRepository(
            'KisaanMessageBundle:Message'
        )->getNbUnreadMessage($user, $type);

        return $this->render(
            'KisaanUserBundle:Dashboard/Profile:profile_switch.html.twig',
            array(
    ...
    private function createProfileSwitchForm(Request $request)
    {
        ...
        $form = $this->get('form.factory')->createNamed(
            ...
            array(
                'method' => 'POST',
                'action' => $this->generateUrl('kisaan_user_dashboard_profile_switch'),
