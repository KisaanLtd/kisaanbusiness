...
namespace Kisaan\ReviewBundle\Controller\Dashboard;

use Kisaan\CoreBundle\Entity\Booking;
use Kisaan\ReviewBundle\Entity\Review;
use Kisaan\ReviewBundle\Form\Type\Dashboard\ReviewType;
...
class ReviewController extends Controller
{
     ...
     *
     * @Route("/new/{booking_id}", name="kisaan_dashboard_review_new")
     *
     * @Method({"GET", "POST"})
    ...
    {
        $user = $this->getUser();
        $formHandler = $this->get('kisaan.form.handler.review');
        ...
        if ($submitted !== false) {
            $this->get('session')->getFlashBag()->add(
                'success',
                $translator->trans('review.new.success', array(), 'kisaan_review')
            );

            return $this->redirect($this->generateUrl('kisaan_dashboard_reviews_made'));
        }

        return $this->render(
            'KisaanReviewBundle:Dashboard/Review:new.html.twig',
            array(
     ...
     *
     * @Route("/reviews-made", name="kisaan_dashboard_reviews_made")
     *
     * @Method({"GET"})
     *
     * @param  Request $request
    ...
    public function madeReviewsAction(Request $request)
    {
        ...
        $reviewManager = $this->get('kisaan.review.manager');
        ...
        return $this->render(
            'KisaanReviewBundle:Dashboard/Review:index.html.twig',
            array(
                'reviews' => $madeReviews,
     ...
     *
     * @Route("/reviews-received", name="kisaan_dashboard_reviews_received")
     *
     * @Method({"GET"})
     *
     * @param  Request $request
