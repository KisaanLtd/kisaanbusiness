...
namespace Kisaan\BreadcrumbBundle\Services;
...
class BreadcrumbsManager implements TranslationContainerInterface
{
        ...
        if (isset($breadcrumbLinks[$routeName]) && is_array($breadcrumbLinks[$routeName])) {
            ...
            foreach ($breadcrumbLinks[$routeName] as $breadcrumb) {
                ...
                if ($text) {
                    ...
                    /** @Ignore */
                        $this->translator->trans($text, array(), 'kisaan_breadcrumbs'),
                        $url
                    );
                }
            }
    ...
    {
        $this->breadcrumbs->addItem(
            $this->translator->trans('Home', array(), 'kisaan_breadcrumbs'),
            $this->router->generate('kisaan_home')
        );

        if ($forDashboard) {
            //Asker or offerer
            $url = $this->router->generate('kisaan_dashboard_message');
            if ($request->getSession()->get('profile', 'asker') == 'asker') {
                $this->breadcrumbs->addItem(
                    $this->translator->trans('Asker', array(), 'kisaan_breadcrumbs'),
                    $url
                );
            } else {
                $this->breadcrumbs->addItem(
                    $this->translator->trans('Offerer', array(), 'kisaan_breadcrumbs'),
                    $url
                );
            }
        }
    ...
    public function addThreadViewItems(Request $request, $threadObj, $user)
    {
        ...
        $this->breadcrumbs->addItem(
            $this->translator->trans('Messages', array(), 'kisaan_breadcrumbs'),
            $this->router->generate('kisaan_dashboard_message')
        );
    ...
    public function addListingShowItems(Request $request, $listing)
    {
        ...
        $this->breadcrumbs->addItem(
            $this->translator->trans('Search results', array(), 'kisaan_breadcrumbs')
        );
        ...
        foreach ($places as $place) {
            ...
            if ($placeEntity = $coordinate->$coordinateMethod()) {
                ...
                if ($geocoding = $placeEntity->getGeocoding()) {
                    ...
                    $url = $this->router->generate('kisaan_listing_search_result', $urlParams);
                }
    ...
    public function addListingResultItems(Request $request, $listingSearchRequest)
    {
        ...
        if ($location->getCountry()) {
            $repo = $this->em->getRepository("KisaanGeoBundle:Country");
            $country = $repo->findOneByCode($location->getCountry());

            if ($country && $location->getArea()) {
                $repo = $this->em->getRepository("KisaanGeoBundle:Area");
...
//                $urlParams['location[country]'] = $location->getCountry();
                ...
                if ($area && $location->getDepartment()) {
                    $repo = $this->em->getRepository("KisaanGeoBundle:Department");
...
//                    $urlParams['location[area]'] = $location->getArea();
                    ...
                    if ($department && $location->getCity()) {
                        $repo = $this->em->getRepository("KisaanGeoBundle:City");
...
//                        $urlParams['location[department]'] = $location->getDepartment();
    ...
    public function addBookingNewItems(Request $request, $booking)
    {
        ...
        $url = $this->router->generate('kisaan_listing_show', array('slug' => $listing->getSlug()));;
        $this->breadcrumbs->addItem($listing->getTitle(), $url);
        $this->breadcrumbs->addItem($this->translator->trans('Reservation', array(), 'kisaan_breadcrumbs'));
    }
    ...
    public function addProfileShowItems(Request $request, $user)
    {
        $this->addPreItems($request, false);
        $this->breadcrumbs->addItem(
            $this->translator->trans('Profile', array(), 'kisaan_breadcrumbs')
        );
    ...
    {
        $messages[] = new Message("Messages", 'kisaan_breadcrumbs');
        $messages[] = new Message("Payments", 'kisaan_breadcrumbs');
        $messages[] = new Message("Comments", 'kisaan_breadcrumbs');
        $messages[] = new Message("Profile", 'kisaan_breadcrumbs');
        $messages[] = new Message("About me", 'kisaan_breadcrumbs');
        $messages[] = new Message("Payment Information", 'kisaan_breadcrumbs');
        $messages[] = new Message("Contact Information", 'kisaan_breadcrumbs');
