...
namespace Kisaan\GeoBundle\Form\DataTransformer;
...
class GeocodingToCoordinateEntityTransformer implements DataTransformerInterface
{
    ...
    public function reverseTransform($geocodingI18n)
    {
        ...
        $coordinate = $this->om->getRepository('KisaanGeoBundle:Coordinate')->findOneBy(
            array(
...
//        die();
    ...
    {
        $country = $this->om->getRepository('KisaanGeoBundle:Country')->findOneBy(
            array(
                'code' => $code
            )
        );
    ...
    private function getOrCreateArea($names, $country)
    {
        $area = null;
        if ($country->getId()) {
            $area = $this->om->getRepository('KisaanGeoBundle:Area')->findOneByNameAndCountry(
                $names[$this->locale],
                $country
            );
        }
    ...
    private function getOrCreateDepartment($names, $area)
    {
        $department = null;
        if ($area->getId()) {
            $department = $this->om->getRepository('KisaanGeoBundle:Department')->findOneByNameAndArea(
                $names[$this->locale],
                $area
            );
        }
    ...
    private function getOrCreateCity($names, $department)
    {
        $city = null;
        if ($department->getId()) {
            $city = $this->om->getRepository('KisaanGeoBundle:City')->findOneByNameAndDepartment(
                $names[$this->locale],
                $department
            );
        }
