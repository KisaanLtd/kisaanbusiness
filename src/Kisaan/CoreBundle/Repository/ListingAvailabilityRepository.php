...
namespace Kisaan\CoreBundle\Repository;
...
//        echo $start->format('Y-m-d H:i') . ' /' .  $end->format('Y-m-d H:i') . '<br>';
        ...
        $qbDM = $this->dm->createQueryBuilder('KisaanCoreBundle:ListingAvailability')
            ->hydrate($hydrate)
     ...
     *
     * @return \Kisaan\CoreBundle\Document\ListingAvailability[]
    ...
    private function findAvailabilitiesInNoDayMode(
        ...
        foreach ($daysTimeRanges as $dayTimeRanges) {
            ...
            if (is_numeric($priceMin) && $priceMin && is_numeric($priceMax) && $priceMax) {//not use for now
                ...
                $embeddedQbDM = $this->dm->createQueryBuilder('KisaanCoreBundle:ListingAvailabilityTime');
