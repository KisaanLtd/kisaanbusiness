...
class LanguageManager
{
    ...
    public function getLanguageLinks($routeName, $routeParams = array(), $queryString)
    {
        ...
        if ($routeName == 'kisaan_listing_show') {
            $slugs = $this->setTranslatedSlugs('KisaanCoreBundle:Listing', $routeParams);
        } //Get slug translations to generate correct page_show url for each languages
        elseif ($routeName == 'kisaan_page_show') {
            $slugs = $this->setTranslatedSlugs('KisaanPageBundle:Page', $routeParams);
        }
