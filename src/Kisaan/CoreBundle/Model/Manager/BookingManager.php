...
namespace Kisaan\CoreBundle\Model\Manager;
...
use Kisaan\CoreBundle\Event\BookingAmountEvent;
use Kisaan\CoreBundle\Event\BookingAmountEvents;
use Kisaan\CoreBundle\Event\BookingEvent;
use Kisaan\CoreBundle\Event\BookingEvents;
use Kisaan\CoreBundle\Event\BookingPayinRefundEvent;
use Kisaan\CoreBundle\Event\BookingValidateEvent;
use Kisaan\CoreBundle\Mailer\TwigSwiftMailer;
use Kisaan\CoreBundle\Repository\BookingRepository;
use Kisaan\CoreBundle\Repository\ListingAvailabilityRepository;
use Kisaan\CoreBundle\Repository\ListingDiscountRepository;
use Kisaan\SMSBundle\Twig\TwigSmser;
use Kisaan\TimeBundle\Model\DateTimeRange;
use Kisaan\TimeBundle\Model\TimeRange;
...
class BookingManager extends BaseManager
{
    ...
    public function __construct(
        ...
        $this->timesMax = $parameters["kisaan_time_max"];
        $this->hoursAvailable = $parameters["kisaan_time_hours_available"];
        ...
        $this->expirationDelay = $parameters["kisaan_booking_expiration_delay"];
        $this->acceptationDelay = $parameters["kisaan_booking_acceptation_delay"];
        $this->minStartTimeDelay = $parameters["kisaan_booking_min_start_time_delay"];
        $this->minPrice = $parameters["kisaan_booking_price_min"];

        $this->maxPerPage = $parameters["kisaan_dashboard_max_per_page"];
        $this->defaultListingStatus = $parameters["kisaan_listing_availability_status"];
        $this->vatRate = $parameters["kisaan_vat"];
        $this->includeVat = $parameters["kisaan_include_vat"];
        $this->bundles = $parameters["kisaan_bundles"];
        $this->invoiceBegin = $parameters["kisaan_booking_invoice_begin"];
    }
...
//            echo $timeRange->getStartMinute() . ' / ' . $timeRange->getEndMinute() . '<br>';
    ...
    private function getBookingDiscount(Booking $booking)
    {
        ...
        $listingDiscountRepository = $this->em->getRepository("KisaanCoreBundle:ListingDiscount");
...
//                throw new \Exception($e);
    ...
    {
        return isset($this->bundles["KisaanMangoPayBundle"]);
    }
    ...
    {
        return isset($this->bundles["KisaanVoucherBundle"]);
    }
    ...
    {
        return $this->em->getRepository('KisaanCoreBundle:Booking');
    }
    ...
    {
        return $this->dm->getRepository('KisaanCoreBundle:ListingAvailability');
    }
