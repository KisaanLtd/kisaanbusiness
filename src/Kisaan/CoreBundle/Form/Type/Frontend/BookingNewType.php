...
use Kisaan\CoreBundle\Event\BookingFormBuilderEvent;
use Kisaan\CoreBundle\Event\BookingFormEvents;
use Kisaan\CoreBundle\Model\Manager\BookingManager;
use Kisaan\TimeBundle\Form\Type\DateRangeType;
use Kisaan\TimeBundle\Form\Type\TimeRangeType;
use DateInterval;
use DateTime;
use DateTimeZone;
...
class BookingNewType extends AbstractType implements TranslationContainerInterface
{
    ...
    public function __construct(
        ...
        $this->currency = $parameters['kisaan_currency'];
    ...
    public function configureOptions(OptionsResolver $resolver)
    {
        $resolver->setDefaults(
            array(
                'data_class' => 'Kisaan\CoreBundle\Entity\Booking',
    ...
    {
        //todo: factorize kisaan and kisaan_booking error messages : move translation domain to kisaan_booking
        $messages = array();
        $messages[] = new Message(self::$tacError, 'kisaan');
        $messages[] = new Message(self::$messageError, 'kisaan');
        $messages[] = new Message(self::$unavailableError, 'kisaan');
        $messages[] = new Message(self::$amountError, 'kisaan');
        $messages[] = new Message(self::$voucherError, 'kisaan_booking');
        $messages[] = new Message(self::$credentialError, 'kisaan');
        $messages[] = new Message(self::$messageDeliveryInvalid, 'kisaan_booking');
        $messages[] = new Message(self::$messageDeliveryMaxInvalid, 'kisaan_booking');
