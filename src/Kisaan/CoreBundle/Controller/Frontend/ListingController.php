...
use Kisaan\CoreBundle\Form\Type\Frontend\ListingNewType;
...
class ListingController extends Controller
{
     ...
     *
     * @Route("/new", name="kisaan_listing_new")
     *
    ...
    {
        $formHandler = $this->get('kisaan.form.handler.listing');
        ...
        if ($success) {
            ...
            $this->get('session')->getFlashBag()->add(
                'success',
                $this->get('translator')->trans('listing.new.success', array(), 'kisaan_listing')
            );
        ...
        return $this->render(
            'KisaanCoreBundle:Frontend/Listing:new.html.twig',
            array(
                'listing' => $listing,
    ...
    private function createCreateForm(Listing $listing)
    {
        $form = $this->get('form.factory')->createNamed(
            ...
            array(
                'method' => 'POST',
                'action' => $this->generateUrl('kisaan_listing_new'),
            )
        );
     ...
     *
     * @Route("/{slug}/show", name="kisaan_listing_show", requirements={
     *      "slug" = "[a-z0-9-]+$"
     * })
     * @Method("GET")
    ...
    public function showAction(Request $request, Listing $listing = null)
    {
        ...
        $reviews = $this->get('kisaan.review.manager')->getListingReviews($listing);
        ...
        return $this->render(
            'KisaanCoreBundle:Frontend/Listing:show.html.twig',
            array(
                'listing' => $listing,
                'reviews' => $reviews,
            )
    ...
    private function handleSlugChange(Listing $listing, $slug)
    {
        if ($slug != $listing->getSlug()) {
            return $this->redirect(
                $this->generateUrl('kisaan_listing_show', array('slug' => $listing->getSlug())),
                301
            );
        }
