...
use Kisaan\CoreBundle\Event\ListingSearchActionEvent;
use Kisaan\CoreBundle\Event\ListingSearchEvents;
use Kisaan\CoreBundle\Form\Type\Frontend\ListingSearchHomeType;
use Kisaan\CoreBundle\Form\Type\Frontend\ListingSearchResultType;
use Kisaan\CoreBundle\Form\Type\Frontend\ListingSearchType;
...
class ListingSearchController extends Controller
{
     ...
     *
     * @Route("/listing/search_result", name="kisaan_listing_search_result")
     * @Method("GET")
     *
    ...
    public function searchAction(Request $request)
    {
        ...
        $listingSearchRequest = $this->get('kisaan.listing_search_request');
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $results = $this->get("kisaan.listing_search.manager")->search(
            ...
            foreach ($form->getErrors(true) as $error) {
                $this->get('session')->getFlashBag()->add(
                    'error',
                    /** @Ignore */
                    $this->get('translator')->trans($error->getMessage(), $error->getMessageParameters(), 'kisaan')
                );
            }
        }
        ...
        return $this->render(
            $isXmlHttpRequest ?
                '@KisaanCore/Frontend/ListingResult/result_ajax.html.twig' :
    ...
    protected function createSearchResultForm(ListingSearchRequest $listingSearchRequest)
    {
        $form = $this->get('form.factory')->createNamed(
            ...
            array(
                'method' => 'GET',
                'action' => $this->generateUrl('kisaan_listing_search_result'),
            )
        );
    ...
    protected function getMarkers(Request $request, $results, $resultsIterator)
    {
        ...
        $currentCurrency = $this->get('session')->get('currency', $this->getParameter('kisaan.currency'));
    ...
    public function searchHomeFormAction()
    {
        ...
        return $this->render(
            '@KisaanCore/Frontend/Home/form_search.html.twig',
            array(
                'form' => $form->createView(),
            )
        );
    ...
    public function searchFormAction()
    {
        ...
        return $this->render(
            '@KisaanCore/Frontend/Common/form_search.html.twig',
            array(
                'form' => $form->createView(),
            )
        );
     ...
     * searched.
     *
     * @Route("/listing/similar_result/{id}", name="kisaan_listing_similar")
     * @Method("GET")
     *
     * @param  Request $request
     * @param int      $id
    ...
    public function similarListingAction(Request $request, $id = null)
    {
        ...
        return $this->render(
            '@KisaanCore/Frontend/Listing/similar_listing.html.twig',
            array(
                'results' => $results
            )
        );
    ...
    protected function getListingSearchRequest()
    {
        ...
        $listingSearchRequest = $session->has('listing_search_request') ?
            ...
            $this->get('kisaan.listing_search_request');
