...
use Kisaan\CoreBundle\Form\Type\Dashboard\ListingEditAvailabilitiesStatusType;
use Kisaan\CoreBundle\Form\Type\Dashboard\ListingEditAvailabilityStatusType;
use DateTime;
...
class ListingAvailabilityStatusController extends Controller
{
     ...
     *      name="kisaan_dashboard_listing_edit_availabilities_status",
    ...
    {
        $listingAvailabilityHandler = $this->get('kisaan.form.handler.listing_availability.status.dashboard');
        ...
        if ($success == 1) {
            ...
            return $this->redirectToRoute(
                'kisaan_dashboard_listing_edit_availabilities_status',
                array('listing_id' => $listing->getId())
            );
        } elseif ($success == -1) {
            $this->get('session')->getFlashBag()->add(
                'error',
                $this->get('translator')->trans('form.error', array(), 'kisaan')
            );
        }

        return $this->render(
            'KisaanCoreBundle:Dashboard/Listing:edit_availabilities_status.html.twig',
            array(
                'listing' => $listing,
     ...
     *      name="kisaan_dashboard_listing_edit_availability_status",
     *      requirements={
     *          "listing_id" = "\d+",
    ...
    public function editAvailabilityStatusAction(
        ...
        if ($success == 1) {
            $this->get('session')->getFlashBag()->add(
                'success',
                $this->get('translator')->trans('listing.availability.edit.success', array(), 'kisaan_listing')
            );
        } elseif ($success == -1) {
            $this->get('session')->getFlashBag()->add(
                'error',
        ...
        return $this->render(
            'KisaanCoreBundle:Dashboard/Listing:form_availability_status.html.twig',
            array(
                'form' => $form->createView(),
            )
        );
    ...
    private function createEditAvailabilityStatusForm($listingId, $day, $startTime, $endTime)
    {
        $form = $this->get('form.factory')->createNamed(
            ...
            array(
                'method' => 'POST',
                'action' => $this->generateUrl(
                    'kisaan_dashboard_listing_edit_availability_status',
                    array(
     ...
     *      name="kisaan_dashboard_listing_new_availability_status",
     *      requirements={
     *          "listing_id" = "\d+",
     *          "day"= "^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$"
     *      },
    ...
    public function newAction(Request $request, Listing $listing, $day)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $this->get("kisaan.listing_availability.manager")->save($form->getData());

            $this->get('session')->getFlashBag()->add(
                'success',
                $this->get('translator')->trans('listing.availability.new.success', array(), 'kisaan_listing')
            );
        }
    ...
    private function createCreateForm(ListingAvailability $availability)
    {
        $form = $this->get('form.factory')->createNamed(
            ...
            array(
                'method' => 'POST',
                'action' => $this->generateUrl(
                    'kisaan_dashboard_listing_new_availability_status',
                    array(
