...
class BookingController extends Controller
{
     ...
     *
     * @Route("/{page}", name="kisaan_dashboard_booking_asker", defaults={"page" = 1})
     * @Method("GET")
     *
     ...
     *
     * @Route("/{id}/show", name="kisaan_dashboard_booking_show_asker", requirements={
     *      "id" = "\d+",
     * })
     * @Method({"GET", "POST"})
    ...
    public function showAction(Request $request, Booking $booking)
    {
        ...
        $canBeCanceledByAsker = $this->get('kisaan.booking.manager')->canBeCanceledByAsker($booking);
     ...
     *
     * @Route("/{id}/edit/{type}", name="kisaan_dashboard_booking_edit_asker", requirements={
     *      "id" = "\d+",
     *      "type" = "cancel",
     * })
     * @Method({"GET", "POST"})
    ...
    {
        $bookingHandler = $this->get('kisaan.form.handler.booking.asker.dashboard');
        $bookingRefundManger = $this->get('kisaan.booking_payin_refund.manager');
        ...
        if ($success == 1) {
            ...
            return $this->redirectToRoute(
                'kisaan_dashboard_booking_edit_asker',
                array(
                    'id' => $booking->getId(),
                    'type' => $type
                )
    ...
    private function createBookingFilterForm()
    {
        $form = $this->get('form.factory')->createNamed(
            ...
            array(
                'action' => $this->generateUrl(
                    'kisaan_dashboard_booking_asker',
                    array('page' => 1)
                ),
                'method' => 'GET',
            )
    ...
    /**
     *
     * @Route("/{id}/show-voucher", name="kisaan_dashboard_booking_show_voucher", requirements={
     *      "id" = "\d+"
     * })
     * @Method("GET")
     *
    ...
    {
        return $this->render(
            'KisaanCoreBundle:Dashboard/Booking:show_voucher.html.twig',
            array(
                'booking' => $booking
            )
        );
