...
abstract class CommonContext extends RawMinkContext implements Context, KernelAwareContext
{
    ...
    {
        return $this->getService('kisaan.repository.' . $resource);
    }
     ...
     * with "kisaan_" and used as route name passed to router generate method.
     *
    ...
    protected function generatePageUrl($page, array $parameters = array())
    {
        ...
        if (null === $routes->get($route)) {
            $route = 'kisaan_' . $route;
            $locale = $this->getContainer()->getParameter('kisaan.locale');
            $route = $locale . \JMS\I18nRoutingBundle\Router\I18nLoader::ROUTING_PREFIX . $route;
            //echo($route);
        }

        if (null === $routes->get($route)) {
            $route = str_replace('kisaan_', 'kisaan_user_', $route);
        }
    ...
    public function purgeMongoDatabase(BeforeScenarioScope $scope)
    {
        ...
        $documentManager->createQueryBuilder('KisaanCoreBundle:ListingAvailability')
            ->remove()
