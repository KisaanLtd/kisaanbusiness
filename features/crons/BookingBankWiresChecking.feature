...
Feature: Bookings bank wires checking command
  ...
  Scenario: I can check that a bank wire has been transferred to an offerer after a booking has been payed
    ...
    When I am logged in as user "offerer@kisaan.rocks" with password "12345678"
    ...
    #Booking validations
    When I run "kisaan:bookings:validate" command
    Then I should see "1 booking(s) validated" on console
    #Bank wire
    When I am logged in on admin as user "super-admin@kisaan.rocks" with password "super-admin"
    And Admin make a bank wire on offerer bank account
    And I run "kisaan:bookings:checkBankWires" command
    Then I should see "1 booking(s) bank wires checked" on console
    And He should receive the "booking_bank_wire_transfer_offerer" mail on "offerer@kisaan.rocks"