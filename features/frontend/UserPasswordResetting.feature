...
Feature: User password resetting
  ...
  Scenario: I can request a new password and receive a resetting email with a link to change it
    ...
    And I should receive the "forgot_password_user" mail on "offerer@kisaan.rocks"
    When I wait 2000 ms
    And I follow the resetting reset link in the "forgot_password_user" mail send to "offerer@kisaan.rocks"
    Then I should be on the resetting password page with token of "offerer@kisaan.rocks" user
    When I fill in the following:
