...
class UserContext extends CommonContext
{
     ...
     */
    public function iAmLoggedInUser($email = 'user@kisaan.rocks', $password = '')
    {
     ...
     * @param string $password
     */
    private function iAmLoggedInAsRole($role, $email = 'user@kisaan.rocks', $password = '')
    {
        if (!$password) {
    ...
    public function thereIsUser(
        ...
        /** @var  $userManager \Kisaan\UserBundle\Model\UserManager */
        $userManager = $this->getContainer()->get('kisaan_user.user_manager');
...
//            echo $this->generatePageUrl("resetting reset", array("token" => $user->getConfirmationToken())). "\n";
    ...
    public function iShouldBeOnTheUserPage($page, $email)
    {
        ...
        /** @var $entity \Kisaan\UserBundle\Entity\User */
        $entity = $em->getRepository('KisaanUserBundle:User')->findOneByEmail($email);
        if (null !== $entity) {
            $this->assertSession()->addressEquals(
